...
function ActivityNewbeeFund:initUIComponent()
	if self.isNew_ then
		...
		xyd.setUISpriteAsync(self.imgText2_, nil, "activity_newbee_fund_popup_logo_" .. xyd.Global.lang)

		self.desLabel_.text = __("ACTIVITY_NEWBEE_FUND_TEXT01")
		self.awardNumLabel.text = "30000"
		fundTable = xyd.tables.activityNewbeeFundTable2
	else
		self.imgText2_.gameObject:SetActive(false)
		xyd.setUITextureByNameAsync(self.imgText_, "activity_newbee_fund_logo_" .. xyd.Global.lang, true)
		...
		fundTable = xyd.tables.activityNewbeeFundTable
	end
...
function ActivityNewbeeFund:initBuyGroup()
	...
	print("giftBagID:", self.giftBagID)
...
function ActivityNewbeeFund:onRegister()
	...
	UIEventListener.Get(self.checkBtn).onClick = function ()
		xyd.WindowManager.get():openWindow("activity_newbee_fund_award_window", {
			isNew = self.isNew_
		})
